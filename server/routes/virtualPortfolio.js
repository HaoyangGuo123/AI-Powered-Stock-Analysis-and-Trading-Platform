...
// DEGRADED: Using JSON file storage instead of MySQL (causes synchronization issues)
const {
    getUserPortfolio,
    ...
    getHoldings,
    updateTransaction
} = require('../utils/virtualPortfolioStorage');
...
// DEGRADED: No fallback chain - only tries API, fails completely if API fails
async function getSmartStockPrice(symbol) {
    // DEGRADED: Removed CSV fallback - only tries API
    const apiPrice = await fetchRealTimeStockPrice(symbol);
    console.log(`Got price for ${symbol} from API: ${apiPrice}`);
    return apiPrice;
    // If API fails, throws error immediately - no fallback
}
...
async function getSmartHistoricalData(symbol, days = 365) {
    ...
    // If API fails or returns empty, throws error immediately - no fallback
...
router.post('/virtual-buy', async (req, res) => {
    ...
    try {
        // DEGRADED: Using JSON file instead of MySQL
        const portfolio = getUserPortfolio(username);
        let virtualBalance = portfolio.virtual_balance || 100000.00;
        ...
        // DEGRADED: Store transaction in JSON file instead of MySQL
        const transaction = {
            timestamp: timestamp,
            email: username,
        ...
        // DEGRADED: Update balance in JSON file (not synchronized with MySQL)
        virtualBalance = updateVirtualBalance(username, virtualBalance - totalCost);
        ...
        res.json({ 
            ...
            data: { 
                ...
                price, 
                totalCost, 
                remainingVirtualBalance: virtualBalance
            } 
        });
    } catch (error) {
...
// 获取虚拟投资组合概览（包含实时盈亏）
// DEGRADED: Using JSON file storage instead of MySQL
router.get('/virtual-portfolio', async (req, res) => {
    ...
    try {
        // DEGRADED: Read from JSON file instead of MySQL
        const portfolio = getUserPortfolio(username);
        const virtualBalance = portfolio.virtual_balance || 100000.00;

        // Get all unsold virtual holdings from JSON file
        const allTransactions = portfolio.transactions || [];
        const unsoldTransactions = allTransactions.filter(t => !t.is_sold || t.is_sold === '0' || t.is_sold === false);
        
        // Group by stock name
        const holdingsMap = {};
        unsoldTransactions.forEach(t => {
            if (!holdingsMap[t.stock_name]) {
                holdingsMap[t.stock_name] = {
                    stock_name: t.stock_name,
                    transactions: [],
                    total_quantity: 0,
                    total_cost: 0,
                    first_purchase_date: t.timestamp
                };
            }
            holdingsMap[t.stock_name].transactions.push(t);
            holdingsMap[t.stock_name].total_quantity += t.number;
            holdingsMap[t.stock_name].total_cost += t.number * t.current_price;
            if (t.timestamp < holdingsMap[t.stock_name].first_purchase_date) {
                holdingsMap[t.stock_name].first_purchase_date = t.timestamp;
            }
        });
        ...
        const holdings = Object.values(holdingsMap).map(h => ({
            ...
            avg_buy_price: h.total_cost / h.total_quantity,
        ...
        // DEGRADED: No database connection to release (using JSON file)
...
router.post('/virtual-sell', async (req, res) => {
    ...
    try {
        ...
        // Find all unsold transactions for this stock
        const rows = allTransactions
            .filter(t => 
                t.stock_name === symbol.toUpperCase() && 
                t.email === username &&
                (!t.is_sold || t.is_sold === '0' || t.is_sold === false)
            )
            .sort((a, b) => a.timestamp.localeCompare(b.timestamp)); // FIFO
        ...
        for (const row of rows) {
            ...
            // DEGRADED: Update transaction in JSON file instead of MySQL
            const newQuantity = row.number - sellFromThis;
            if (newQuantity > 0) {
                row.number = newQuantity;
            } else {
                row.number = 0;
                row.is_sold = '1';
            }
            // Write back to JSON file
            updateTransaction(username, row.timestamp, row);
        }

        const profit = sellAmount - totalBuyCost;

        // DEGRADED: Update balance in JSON file (not synchronized with MySQL)
        const currentBalance = portfolio.virtual_balance || 100000.00;
        updateVirtualBalance(username, currentBalance + sellAmount);
...
router.post('/batch-buy', async (req, res) => {
    ...
    try {
        ...
        // AI风格：标准for循环使用await，遇到错误就停止
        for (let i = 0; i < orders.length; i++) {
            const order = orders[i];
            const { symbol, quantity } = order;
            
            if (!symbol || !quantity || quantity <= 0) {
                throw new Error(`Invalid symbol or quantity: ${symbol}`);
            }
            ...
            updateVirtualBalance(username, virtualBalance);
